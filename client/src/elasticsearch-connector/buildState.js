...
const buildTotalPages = (resultsPerPage, totalResults) => {
  if (!resultsPerPage) return 0;
  if (totalResults === 0) return 1;
  return Math.ceil(totalResults / resultsPerPage);
};

const buildTotalResults = hits => {
  return hits.total.value;
};

const getHighlight = (hit, fieldName) => {
...
*/
export default (response, resultsPerPage) => {
